...
class m161212_073220_settings_terminals extends Migration
{
    public function up()
    {
        ...
        foreach($allSettings as $model) {
            ...
            foreach($terminals as $terminal) {
                ...
                if (!$result) {
                    // Если процессы не выключены, то регулярные задания могли уже насоздавать новых моделей
        ...
        // Во всех AppSettings оставить данные только об их собственном терминале
        $appSettings = SettingsAR::findAll(['code' => 'app']);

        foreach($appSettings as $model) {

            if (!$model->terminalId) {
                // пропустить дефолтные модели
                continue;
            }
    ...
    {
        // Собрать данные о терминалах из всех AppSettings, кроме дефолтного назад в общую модель
        ...
        $defaultTerminalId = Yii::$app->exchange->defaultTerminal->terminalId;
        ...
        // Удалить все недефолтные модели
