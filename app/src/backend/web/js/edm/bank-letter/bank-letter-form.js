...
function AttachedFilesListController(
    ...
    this.bindToView = function () {
        ...
        this.$storageInput.on('change', function() {
            self.renderGridView();
        });

        this.$form.closest('.modal').on('hide.bs.modal', function () { self.reset() });

        this.$form.on('update-max-attachments-count', function(event, maxCount) {
            self.updateMaxAttachmentsCount(maxCount);
        });
    };
    ...
    this.saveList = function (list) {
        ...
        this.$storageInput.trigger('change');
    };
    ...
    this.renderGridView = function (onComplete) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                self.checkMaxAttachmentsCount();
            }
        });
    };

    this.updateMaxAttachmentsCount = function (maxCount) {
        this.maxAttachmentsCount = maxCount;
        this.checkMaxAttachmentsCount();
    };

    this.checkMaxAttachmentsCount = function () {
        var canAddMoreAttachments = !this.maxAttachmentsCount || this.getList().length < this.maxAttachmentsCount;
        this.$addButton.toggleClass('hidden', !canAddMoreAttachments)
    };
}
...
var BankLetterForm = {
    ...
    initialize: function () {
        AttachedFilesListController.run({
            ...
            renderGridViewUrl:    '/edm/bank-letter/render-attached-files'
        });
    }

};

BankLetterForm.initialize();
