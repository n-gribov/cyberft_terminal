...
optionSwitcher = (function(e, radioGroupName) {
	if (e) { // Обработка события - нажатие на радиоэлемент управления
		...
		if (myVisibleDivs.length == 1)
		{
			...
			if(changedCnt>0)
			{
				if (confirm('При переключении опции введенные данные будут потеряны. Продолжить?') == false)
...
mtOptionsInit = (function() {
	// Устанавливаем обработчики событий для каждого из переключателей опций
	var myRadioGroups = $("input[class~='options-switcher'][type='radio']");
	myRadioGroups.on('click',optionSwitcher);
