package CyberFT::SWIFT::Types::MT545;
    ...
    rules => [
        ...
        # подпоследовательности Е3 «Суммы» (Код ошибки Е87):
        {
            func => sub {
                ...
                my @codes = qw(
                    ACRU ANTO CHAR COMT COUN DEAL EXEC ESTT ISDI LADT LEVY LOCL LOCO
        ...
        # С2
        # В сообщении обязательно должна быть указана «Сумма проведенных расчетов»: в одном
        # повторении подпоследовательности Е3 «Суммы» должно присутствовать поле :19A::ESTT
        # (Код ошибки Е92).
        {
            func => sub {
                my $doc = shift;
                ...
                my $branches_e3 = $tree->{'SETDET'}->[0]->{'AMT'} || [];
                for my $branch_e3 (@$branches_e3) {
                    if (seq_key_value_exists($branch_e3->{fields}, '19A', ":ESTT")) {
                        return 1;
                    }
                }
                return 0;
        ...
        # подпоследовательности Е1 «Стороны при расчетах» должно присутствовать поле :95a::DEAG, а в
        # другом повторении – поле :95a::PSET (Код ошибки Е91).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
        ...
        # С7
        # Если сообщение содержит отмену или отзыв, т.е. если поле 23 G «Функция сообщения» содержит
        # код CANC или RVSL, то в одном и только в одном повторении последовательности связок А1 должно
        ...
        # в одном повторении последовательности связок А1 должно присутствовать поле :20C::RELA; в
        # остальных повторениях А1 поле :20C::RELA не может использоваться (Код ошибки С73).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
        ...
        # С10
        # В последовательности C поле :36B::ESTT не может использоваться более двух раз. Если это
        # поле используется дважды, то в первом случае «Код типа количества» должен быть «FAMT», а
        # во втором случае «Код типа количества» должен быть «AMOR» (Код ошибки С71).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
        ...
        # С11
        # Дата валютирования должна быть указана только для суммы фактически проведенных расчетов,
        # то есть, если в любом повторении подпоследовательности Е3 присутствует поле даты
        # валютирования :98a::VALU, то в той же подпоследовательности Е3 должно присутствовать поле
        # суммы проведенных расчетов :19A::ESTT (Код ошибки С28).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
