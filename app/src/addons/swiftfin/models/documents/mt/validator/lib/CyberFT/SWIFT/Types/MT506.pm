package CyberFT::SWIFT::Types::MT506;
...
our $ValidationProfile = {
    ...
    rules => [
            ...
            func => sub {
                ...
                my $sequences = {
                    'GENL' => {
                        name => 'A',
                        required => 1,
                        required_fields => ['28E', '20C', '23G', '22[FH]', '95[PQR]'],
                    },
                    'GENL/AGRE' => {
                        name => 'A1',
                        required => 1,
                    ...
                    'SUMM' => {
                        name => 'B',
                        required => 1,
                        required_fields => ['95[PQR]', '19B', '98[AC]'],
                    },
                    'SUMM/SUMD' => {
                        name => 'B1',
                        required => 0,
                    ...
                    'EXPD' => {
                        name => 'C',
                        required => 0,
                        required_fields => ['22[FH]', '19A', '92[AB]'],
                    },
                    'EXPD/SECS' => {
                        name => 'C1',
                        required => 0,
                        required_fields => ['19B', '35B', '36B', '92A'],
                    },
                    'EXPD/TREA' => {
                        name => 'C2',
                        required => 0,
                    ...
                    'EXPD/REPV' => {
                        name => 'C3',
                        required => 0,
                        required_fields => ['98A', '19B', '92A'],
                    },
                    'COLD' => {
                        name => 'D',
                        required => 0,
                        required_fields => ['20C', '22H', '19B', '92[AB]'],
                    },
                    'COLD/SCOL' => {
                        name => 'D1',
                        required => 0,
        ...
        # код CANC, в сообщении должна присутствовать хотя бы одна подпоследова- тельность А2
        # «Связки», и в одном и только в одном повторении А2 должно присутствовать поле :20C::PREV.
        ...
        # С4
        # В каждом повторении последовательности D использование подпоследовательностей D1, D2 и D3
        # зависит от значения поля :22H::COLL//«Признак» следующим образом (Код ошибки Е83):
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);
                my $branches_d = $tree->{'COLD'} || [];
        ...
        # С5
        # В каждом повторении подпоследовательности D2 использование поля :98A::MATU зависит от
        # значения поля :22H::DEPO//«Признак» следующим образом (Код ошибки Е85):
        {
            func => sub {
                ...
                my $branches_d = $tree->{'COLD'} || [];
                for my $bd (@$branches_d) {
                    my $branches_d2 = $bd->{'CCOL'} || [];
                    for my $bd2 (@$branches_d2) {

                        if (seq_key_value_exists($bd2->{fields}, '22H', ':DEPO//FIXT')) {
                            return 0 unless (seq_key_value_exists($bd2->{fields}, '98A', ':MATU'));
                        }
                        elsif (seq_key_value_exists($bd2->{fields}, '22H', ':DEPO//CLNT')) {
                            return 0 if (seq_key_value_exists($bd2->{fields}, '98A', ':MATU'));
                        }
        ...
        # С6
        # В любом повторении подпоследовательности D3, если в нем присутствует поле :22H::BCOL//LCOL,
        ...
        # поле :98B::EXPI//OPEN необязательное (Код ошибки Е72):
        {
            func => sub {
                ...
                my $branches_d = $tree->{'COLD'} || [];
                for my $bd (@$branches_d) {
                    my $branches_d3 = $bd->{'BCOL'} || [];
                    for my $bd3 (@$branches_d3) {

                        if (seq_key_value_exists($bd3->{fields}, '22H', ':BCOL//LCOL')) {
                            return 0 if (seq_key_value_exists($bd3->{fields}, '98B', ':EXPI//OPEN'));
                        }
        ...
        # С10
        # В любом повторении подпоследовательности D1 использование поля :70C::RATS зависит от
        # присутствия поля :94B::RATS следующим образом (Код ошибки Е60):
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);
                ...
                my $branches_d = $tree->{'COLD'} || [];
                for my $bd (@$branches_d) {
                    my $branches_d1 = $bd->{'SCOL'} || [];
                    for my $bd1 (@$branches_d1) {

                        if (seq_key_value_exists($bd1->{fields}, '94B', ':RATS')) {
                            return 0 unless (seq_key_value_exists($bd1->{fields}, '70C', ':RATS'));
                        }
                        else {
                            return 0 if (seq_key_value_exists($bd1->{fields}, '70C', ':RATS'));
                        }
        ...
        # С11
        # Если последовательность С не используется, то последовательность D обязательная.
        # Если в сообщении присутствует хотя бы одна последовательность С, то последовательность D
        # необязательная (Код ошибки С20):
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);
                my $branches_c = $tree->{'EXPD'} || [];
                my $branches_d = $tree->{'COLD'} || [];
                return 0 if (scalar(@$branches_c) == 0 && scalar(@$branches_d) == 0);
                return 1;
            },
            err => 'C20',
        },
