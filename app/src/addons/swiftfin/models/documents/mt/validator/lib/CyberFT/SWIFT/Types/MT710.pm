package CyberFT::SWIFT::Types::MT710;
...
our $ValidationProfile = {
    ...
    rules => [
        # C1   Могут присутствовать либо поле 39A, либо поле 39B, но не оба эти поля вместе (Код
        # ошибки D05).
        {
            func => sub {
                my $doc = shift;
        ...
        # C90).
        {
            func => sub {
                my $doc = shift;
                if($doc->key_exists('42C') or $doc->key_exists('42[AD]') ){
                    if( (not $doc->key_exists('42C')) or (not $doc->key_exists('42[AD]') ) ){
                        return 0;
                    }
                }
                return 1;
        ...
        # C4 Могут присутствовать либо поле 44C, либо поле 44D, но не оба эти поля вместе (Код
        # ошибки D06)
        {
            func => sub {
                my $doc = shift;
        ...
        # C5 Могут присутствовать либо поле 52а «Банк-эмитент», либо поле 50В «Эмитент, не
        # являющийся банком», но не оба эти поля вместе (Код ошибки С06).
        {
            func => sub {
                my $doc = shift;
                if($doc->key_exists('52[AD]') and $doc->key_exists('50B')){
                    return 0;
                }
                return 1;
            },
