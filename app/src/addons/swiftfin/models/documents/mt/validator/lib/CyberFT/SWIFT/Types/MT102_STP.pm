package CyberFT::SWIFT::Types::MT102_STP;
...
our $ValidationProfile = {
    ...
    rules => [
        ...
        # C5
        # Если любое из полей 52A, 26Т или 77В присутствует в последовательности А, то это поле не
        # должно присутствовать ни в одном из повторений последовательности В. И наоборот, если любое
        # из полей 52A, 26Т или 77В присутствует в одном или более из повторений последовательности
        # В, то оно не должно присутствовать в последовательности А (Код ошибки D18).
        {
            func => sub {
                ...
                my $a = _find_A_seq($doc->{data});
                my $bs = _find_B_seqs($doc->{data});
                if (seq_key_exists($a, '52A')) {
                    for my $b (@$bs) {
                        return 0 if (seq_key_exists($b, '52A'));
                    }
                }
        ...
        # SI, SJ, SK, SM, TF и VA, - то для каждого из повторений последовательности В должны
        # выполняться следующие правила:
        # - Если отсутствует поле 57А, то в подполе «Счет» поля 59а этого повторения
        # последовательности В должен быть указан Международный номер банковского счета IBAN
        # (стандарт ISO-13616) (Код ошибки D19).
        # - Если поле 57А присутствует, и если указанный в нем код BIC содержит код страны,
        # входящий в приведенный выше перечень, то в подполе «Счет» поля 59а этого повторения
        # последовательности В должен быть указан Международный номер банковского счета IBAN
        # (стандарт ISO-13616) (Код ошибки D19).
        ...
        # обязательно, и формат подполя «Счет» в поле 59а не проверяется системой SWIFT.
        {
            func => sub {
                ...
                for my $b (@$bs) {
                    ...
                    unless (seq_key_exists($b, '57A')) {
                        $need_to_check_59a = 1;
                    }
                    if (seq_key_exists($b, '57A')) {
                        my $v57a = seq_get_first($b, '57A');
                        $v57a =~ s|^/.*[\r\n]||;
        ...
        # Дополнительные проверки для MT 102 STP
        # {
        #     func => sub {
        #         my $doc = shift;

        #         if (seq_key_exists($doc->{data}, '52[B-Z]?|57[B-Z]?')){
        #             return 0, 'MT 102 STP format validation: Fields 52 and 57 may only be used with letter option A';
        #         }

        #         if (seq_key_exists($doc->{data}, '51A')){
        #             return 0, 'MT 102 STP format validation: Field 51A is not used in MT 102 STP';
        #         }

        #         my $fields23 = seq_get_all($doc->{data}, '23');
        #         for my $val (@$fields23) {
        #             if ($val !~ /^\s*(CREDIT|SPAY)\s*$/) {
        #                 return 0, 'MT 102 STP format validation: Field 23 may only contain codes CREDIT and SPAY ';
        #             }
        #         }

        #         my $fields59a = seq_get_all($doc->{data}, '59A?');
        #         for my $val (@$fields59a) {
        #             if ($val !~ m|^/\S+|) {
        #                 return 0, 'MT 102 STP format validation: Subfield 1 (Account) of field 59a is always mandatory';
        #             }
        #         }

        #         return 1;
        #     },
        #     err => 'MT 102 STP format validation failed',
        # }
    ]
