package CyberFT::SWIFT::Types::MT541;
    ...
    rules => [
        ...
        # подпоследовательности Е3 «Суммы» (Код ошибки Е87):
        {
            func => sub {
                ...
                my @codes = qw(
                    ACRU ANTO BOOK CHAR COUN DEAL EXEC ISDI LADT LEVY LOCL LOCO MARG
        ...
        # (Код ошибки Е84):
        {
            func => sub {
                ...
                # F
                for my $code (qw(EXCH MEOR MERE TRRE VEND TRAG BRKR)) {
                    return 0 if (seq_key_value_count($fields, '95[A-Z]', ":$code") > 1);
                }
        ...
        # должны быть указаны агент-поставщик и место проведения расчетов: в одном из повторений
        # подпоследовательности Е1 «Стороны при расчетах» должно присутствовать поле :95a::DEAG,
        # а в другом повторении – поле :95a::PSET (Код ошибки Е91).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
        ...
        # C11
        # Если в последовательности Е присутствует поле :22F::DBNM, то должен быть указан продавец,
        # т.е., в одном из повторений подпоследовательности Е1 должно присутствовать поле :95a::SELL
        # (Код ошибки Е70).
        {
            func => sub {
                my $doc = shift;
                ...
                if (seq_key_value_exists($branch_e->{fields}, '22F', ':DBNM')) {
                    ...
                    for my $b (@$branches_e1) {
                        $counter++ if (seq_key_value_exists($b->{fields}, '95[A-Z]', ':SELL'));
                    }
                    return 0 if ($counter < 1);
                }
