package CyberFT::SWIFT::Types::MT543;
       ...
       # Проверка обязательных последовательностей и полей
        ...
        # подпоследовательности Е3 «Суммы» (Код ошибки Е87):
        {
            func => sub {
                ...
                my @codes = qw(
                    ACRU ANTO CHAR COUN DEAL EXEC ISDI LADT LEVY LOCL LOCO MARG
                    OTHR REGF SETT SHIP SPCN STAM STEX TRAN TRAX VATA WITH COAX ACCA
                );
        ...
        # С2
        # В сообщении обязательно должна быть указана сумма расчетов: хотя бы в одном повторении
        # подпоследовательности Е3 «Суммы» должно присутствовать поле :19A::SETT «Сумма расчетов»
        # (Код ошибки Е92).
        {
            func => sub {
                my $doc = shift;
                ...
                my $branches_e3 = $tree->{'SETDET'}->[0]->{'AMT'} || [];
                for my $branch_e3 (@$branches_e3) {
                    if (seq_key_value_exists($branch_e3->{fields}, '19A', ":SETT")) {
                        return 1;
                    }
                }
                return 0;
        ...
        # расчетных инструкций », то должно присутствовать также и поле :99B::SETT
        # «Порядковый номер расчетных инструкций» (Код ошибки Е90).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
                my $branch_a = $tree->{'GENL'}->[0] || {};
                if (seq_key_value_exists($branch_a->{fields}, '99B', ':TOSE')) {
                    return 0 unless (seq_key_value_exists($branch_a->{fields}, '99B', ':SETT'));
                }
                return 1;
            },
            err => 'E90',
        ...
        # С4
        # Если в подпоследовательности Е3 присутствует поле :92B::EXCH «Курс конвертации», то в той
        # же подпоследовательности должно также присутствовать поле :19A::RESU
        # «Сумма после конвертации». Если поле «Курс конвертации» отсутствует, то поле
        # «Сумма после конвертации» использоваться не может (Код ошибки Е62).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
        ...
        # С6
        # Если в последовательности Е НЕ используется поле :22F::DBNM, то в сообщении обязательно
        # должны быть указаны агент по получению и место проведения расчетов: в одном из повторений
        # подпоследовательности Е1 «Стороны при расчетах» должно присутствовать поле :95a::REAG, а
        # в другом повторении – поле :95a::PSET (Код ошибки Е93).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
                ...
                if (!seq_key_value_exists($branch_e->{fields}, '22F', ':DBNM')) {
                    my ($found_deag, $found_pset);
                    for my $branch_e1 (@$branches_e1) {
                        if (seq_key_value_exists($branch_e1->{fields}, '95[A-Z]', ':REAG')) {
                            $found_deag = 1;
                            next;
                        }
                    ...
                    return 0 unless($found_deag && $found_pset);
                }
        ...
        # (Код ошибки Е08).
        {
            func => sub {
                ...
                elsif (seq_key_value_exists($fields_a, '23G', 'NEWM|PREA')) {
                    my $count = 0;
                    for my $b (@$branches_a1) {
                        if (seq_key_value_exists($b->{fields}, '20C', ':PREV')) {
                            $count++;
        ...
        # последовательности не должно присутствовать поле :97a::SAFE (Код ошибки Е52).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
        ...
        # С10
        # Если в последовательности E присутствует поле :22F::FXCX//FXNO или FXYE, то сообщение
        # должно содержать запрос об отмене, т.е. функция сообщения (поле :23G) в последовательности
        # А должна быть определена как «CANC».
        # Если в последовательности E присутствует поле :22F::FXCX//SINO, то сообщение должно
        # содержать новые инструкции, т.е. функция сообщения (поле :23G) в последовательности А
        # должна быть определена как «NEWM» (Код ошибки Е14).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
                ...
                my $fields_a = $tree->{'GENL'}->[0]->{fields} || [];
                my $fields_e = $tree->{'SETDET'}->[0]->{fields} || [];

                if (seq_key_value_exists($fields_e, '22F', ':FXCX//(FXNO|FXYE)')) {
                    return 0 unless (seq_key_value_exists($fields_a, '23G', 'CANC'))
                }
                if (seq_key_value_exists($fields_e, '22F', ':FXCX//SINO')) {
                    return 0 unless (seq_key_value_exists($fields_a, '23G', 'NEWM'))
                }
        ...
        # C11
        # Если в последовательности Е присутствует поле :22F::DBNM, то должен быть указан покупатель,
        # т.е. в одном из повторений подпоследовательности Е1 должно присутствовать поле :95a::BUYR
        # (Код ошибки Е70).
        {
            func => sub {
                my $doc = shift;
                ...
                if (seq_key_value_exists($branch_e->{fields}, '22F', ':DBNM')) {
                    ...
                    for my $b (@$branches_e1) {
                        $counter++ if (seq_key_value_exists($b->{fields}, '95[A-Z]', ':BUYR'));
                    }
                    return 0 if ($counter < 1);
                }
        ...
        # С12
        # Если в последовательности Е присутствует поле :22F::DBNM//VEND, то должен быть указан
        # поставщик базы данных, т.е., в одном из повторений последовательности F должно
        # присутствовать поле :95a::VEND (Код ошибки Е71).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
                ...
                if (seq_key_value_exists($branch_e->{fields}, '22F', ':DBNM//VEND')) {
                    my $counter = 0;
                    for my $b (@$branches_f) {
        ...
        # С13
        # Если поле :36B: присутствует хотя бы в одном повторении подпоследовательности А1,
        # то тип расчетной операции должен определяться как взаимозачет или оборот, т.е. в
        # последовательности E должно присутствовать поле :22F::SETR//PAIR или :22F::SETR//TURN
        # (Код ошибки С50).
        {
            func => sub {
                my $doc = shift;
                ...
                my $field_36b_exists = 0;
                for my $b (@$branches_a1) {
                    if (seq_key_exists($b->{fields}, '36B')) {
                        $field_36b_exists = 1;
                        last;
                    }
                }
                ...
                if ($field_36b_exists) {
                    return 0 unless (seq_key_value_exists($branch_e->{fields}, '22F', ':SETR//(PAIR|TURN)'))
                }
        ...
        # С14
        # В последовательности C поле :36B::SETT не может использоваться более двух раз. Если это
        # поле используется дважды, то в первом случае Код типа количества должен быть «FAMT», а во
        # втором случае Код типа количества должен быть «AMOR» (Код ошибки С71).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
        ...
        # C15
        # Дата валютирования может быть указана только в случае раздельных расчетов по денежным
        # средствам и ценным бумагам. Т.е. в любой подпоследовательности Е3, если поле дата
        # валютирования :98a::VALU присутствует, тогда в подпоследовательности Е3 также должно
        # присутствовать поле :22F::STCO//SPSTи сумма по расчетам :19A::SETT должно присутствовать
        # в той же подпоследовательности Е3. Код ошибки С28).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
        ...
        # C16
        # Если в последовательности F присутствует поле :95a::EXCH Фондовая биржа или :95a::TRRE
