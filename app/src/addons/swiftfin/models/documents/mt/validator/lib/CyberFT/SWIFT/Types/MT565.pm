...
our $ValidationProfile = {
    ...
    rules => [
        ...
        # С2
        # Если в последовательности D присутствует поле :22F::CAOP//SPLI, то в той же
        # последовательности должно также присутствовать поле :70E::INST (Код ошибки Е79).
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);
        ...
        # С3
        # В подпоследовательности В2 поле :93B::ELIG не может использоваться более двух раз.
        ...
        # С4
        # В последовательности D поле :36B::QINS или QREC не может использоваться более двух раз.
        # Если это поле используется дважды, то в первом случае Код типа количества должен быть
        # «FAMT», а во втором случае Код типа количества должен быть «AMOR» (Код ошибки С72).
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);
                ...
                # Примечание: При использовании опции формата С в поле 36а определители QINS или 
                # QREC повторяться не могут.
