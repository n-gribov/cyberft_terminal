package CyberFT::SWIFT::Types::MT515;
    ...
    rules => [
        ...
        # present, the Resulting Amount is not allowed (Error code(s): E62).
        {
            func => sub {
                ...
                my $branches_d3 = $tree->{'SETDET'}->[0]->{'AMT'} || [];
                for my $branch_d3 (@$branches_d3) {
                    if (seq_key_value_exists($branch_d3->{fields}, '92B', ':EXCH')) {
                        return 0 unless (seq_key_value_exists($branch_d3->{fields}, '19A', ':RESU'));
                    } else {
                        return 0 if (seq_key_value_exists($branch_d3->{fields}, '19A', ':RESU'));
                    }
                }
        ...
        # C2
        # If Settlement Amount (:19A::SETT) is present in sequence C, it must not be present in any
        # occurrence of subsequence D3 (Error code(s): E73).
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);

                my $branch_c = $tree->{'CONFDET'}->[0] || {};
                my $branches_d3 = $tree->{'SETDET'}->[0]->{'AMT'} || [];

                if (seq_key_value_exists($branch_c->{fields}, '19A', ':SETT')) {
                    for my $branch_d3 (@$branches_d3) {
                        return 0 if (seq_key_value_exists($branch_d3->{fields}, '19A', ':SETT'));
                    }
                }
        ...
        # then field :20C::PREV must be present in one and only one occurrence of A1; consequently,
        # in all other occurrences of A1, field :20C::PREV is not allowed (Error code(s): E08).
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);
        ...
        # C4
        # The following party fields for subsequences D1 and D2 cannot appear more than once in sequence D.
        # The party fields for sequence E cannot appear more than once in a message (Error code(s): E84):
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);
        ...
        # C5
        # If a qualifier from the list Deliverers is present in a subsequence D1, in a field :95a::4!c,
        ...
        # must be present (Error code(s): E86).
        {
            func => sub {
                ...
                my $fields_d = _flatten_tree($tree->{'SETDET'}->[0]) || [];
                for my $x1 (keys %$rules) {
                    my $x2 = $rules->{$x1};
                    return 0 if (
                        seq_key_value_exists($fields_d, '95[A-Z]', ":$x1")
                        && !seq_key_value_exists($fields_d, '95[A-Z]', ":$x2")
                    );
                }
        ...
        # C6
        # In subsequence D1, if field :95a::PSET is present, then field :97a::SAFE is not allowed
        # in the same sequence. (Error code(s): E52).
        {
            func => sub {
                ...
                my $branches_d1 = $tree->{'SETDET'}->[0]->{'SETPRTY'} || [];
                for my $branch_d1 (@$branches_d1) {
                    return 0 if (
                        seq_key_value_exists($branch_d1->{fields}, '95[A-Z]', ':PSET')
                        && seq_key_value_exists($branch_d1->{fields}, '97[A-Z]', ':SAFE')
                    );
                }
        ...
        # C7
        # In sequence C, if field :22H::BUSE//SWIT is present, then field :20C::PREV must be
        # present in minimum one occurrence of subsequence A1 Linkages. (Error code(s): E53).
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);
                my $fields_c = $tree->{'CONFDET'}->[0]->{fields} || [];
                my $branches_a1 = $tree->{'GENL'}->[0]->{'LINK'} || [];
                if (seq_key_value_exists($fields_c, '22H', ':BUSE//SWIT')) {
                    return 0 if (scalar @$branches_a1 < 1);
                    my $count = 0;
                    for my $b (@$branches_a1) {
                        if (seq_key_value_exists($b->{fields}, '20C', ':PREV')) {
        ...
        # C8
        # If field :22F::DBNM//VEND is present in sequence D, then a vendor must be specified;
        # that is one occurrence of subsequence E must contain field :95a::VEND (Error code(s): D71).
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);

                my $branch_d = $tree->{'SETDET'}->[0] || {};
                my $branches_e = $tree->{'OTHRPRTY'} || [];

                if (seq_key_value_exists($branch_d->{fields}, '22F', ':DBNM//VEND')) {
                    my $counter = 0;
                    for my $branch_e (@$branches_e) {
                        $counter++ if (seq_key_value_exists($branch_e->{fields}, '95[A-Z]', ':VEND'));
                    }
        ...
        # C9
        # In sequence E, if field :95a::EXCH Stock Exchange or :95a::TRRE Trade Regulator is present,
        ...
        # C10
        # If either Deal Price (field 90a::DEAL) or Original Ordered Quantity (field :36B::ORDR)
        # (in sequence C), or Settlement Amount (field :19A::SETT) (in subsequence D3) are present
        # more than once, then in sequence C Buy/Sell Indicator (field :22H::BUSE) must be present
        # with Indicator code FPOO or IPOO or IPPO (Error code(s): E75).
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);
                ...
                my $fields_c = $tree->{'CONFDET'}->[0]->{fields} || [];
                my $count_90a_deal = seq_key_value_count($fields_c, '90[A-Z]', ':DEAL');
                my $count_36b_ordr = seq_key_value_count($fields_c, '36B', ':ORDR');

                my $branches_d3 = $tree->{'SETDET'}->[0]->{'AMT'} || [];
                my $count_19a_sett = 0;
                for my $branch_d3 (@$branches_d3) {
                    $count_19a_sett += seq_key_value_count($branch_d3->{fields}, '19A', ':SETT');
                }

                if ($count_90a_deal > 1 || $count_36b_ordr > 1 || $count_19a_sett > 1) {
                    return 0 unless (seq_key_value_exists($fields_c, '22H', ':BUSE//(FPOO|IPOO|IPPO)'));
                }
