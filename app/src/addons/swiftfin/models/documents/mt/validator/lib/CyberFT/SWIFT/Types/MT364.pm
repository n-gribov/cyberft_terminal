package CyberFT::SWIFT::Types::MT364;
...
our $ValidationProfile = {
	...
	fields => [
		...
		# M	32B	Current Currency, Notional Amount	3!a15d	See specification
		{
			key => '32B',
			required => 1,
		},
		...
		# O	32G	New Currency, Notional Amount	3!a15d	See specification
	...
	rules => [
		...
		sequence_mandatory_fields_check_rule('A', [qw/20 22A 22B 22C 23A 21N 30T 30Q 30P 30V 32B 82[AD] 87[AD]/]),

		# C7 In all optional sequences, the fields with status M must be present if the sequence is present
		...
		# COLLARBYER	Not allowed	Not allowed
		# COLLARSLLR	Not allowed	Not allowed
		{
			func => sub {
				my $document = shift;
		...
		# C2 The new fixed rates should only be used in the case of recouponing; the new notional amount
		# should only be used in the case of a partial termination. Thus, in sequence A, the presence
		# of fields 32G and 22D and, in sequences B and E, the presence of field 37P depends on field 22B
		# of sequence A as follows (Error code(s): E34):
		#
		# Sequence A       Sequence A         Sequence A         Sequence B         Sequence E
		# if field 22B is  then field 32G is  then field 22D is  then field 37P is  then field 37P
		# PTRC             Mandatory          Mandatory          Mandatory          Mandatory
		# PTRM             Mandatory          Mandatory          Not allowed        Not allowed
		# RCPN             Not allowed        Mandatory          Mandatory          Mandatory
		# TERM             Not allowed        Not allowed        Not allowed        Not allowed
		{
			func => sub {
				my $document = shift;
		...
		# C4 The currency code in all amount fields must be the same (Error code(s): C02).
		{
			func => sub {
				my $document = shift;
				my $amounts = seq_get_all($document->data, '32B|32G|32M');

				my $prev_currency;
				for (@$amounts) {
					if ($_ =~ m/^([A-Z]{3}).+/) {
						unless (defined($prev_currency)) {
							$prev_currency = $1;
							next;
						}
						return 0
							unless $prev_currency eq $1;
					} else {
						return 0;
					}
				}
		...
		# C5 The related reference is required in the case of an amendment or cancellation.
		...
		# C6 In sequence A, if field 22D contains code OTHR, then field 37N must be present (Error code(s): E36).
		{
			func => sub {
				my $document = shift;
