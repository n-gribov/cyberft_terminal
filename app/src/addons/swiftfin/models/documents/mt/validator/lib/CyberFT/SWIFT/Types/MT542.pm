package CyberFT::SWIFT::Types::MT542;
    ...
    rules => [
        ...
        # подпоследовательности Е3 «Суммы» (Код ошибки Е87):
        {
            func => sub {
                ...
                my @codes = qw(
                    ACRU ANTO BOOK CHAR COUN DEAL EUTR EXEC ISDI LADT LEVY LOCL LOCO MARG
        ...
        # C5
        # Если в последовательности Е НЕ используется поле :22FВ::DBNM, то в сообщении обязательно
        # должны быть указаны агент по получению и место проведения расчетов: в одном из повторений
        # подпоследовательности Е1 «Стороны при расчетах» должно присутствовать поле :95a::REAG, а в
        # другом повторении – поле :95a::PSET (Код ошибки Е93).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
        ...
        # С12
        # Если поле :36B::PAIR присутствует хотя бы в одном повторении подпоследовательности А1, то
        # тип расчетной операции должен определяться как взаимозачет, т.е. в последовательности F
        # должно присутствовать поле :22F::SETR//PAIR (Код ошибки С50).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
                ...
                for my $b (@$branches_a1) {
                    if (seq_key_value_exists($b->{fields}, '36B', ':PAIR')) {
                        $field_36b_exists = 1;
                        last;
                    }
                }
                ...
                if ($field_36b_exists) {
                    return 0 unless (seq_key_value_exists($branch_e->{fields}, '22F', ':SETR//PAIR'))
                } else {
                    return 0 if (seq_key_value_exists($branch_e->{fields}, '22F', ':SETR'))
                }
