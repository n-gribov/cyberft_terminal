package CyberFT::SWIFT::Types::MT517;
...
our $ValidationProfile = {
    ...
    rules => [
            ...
            func => sub {
                ...
                my $sequences = {
                    'GENL' => {
                        name => 'A',
                        required => 1,
                        required_fields => ['20C', '23G', '95[PQR]'],
                    },
                    'GENL/LINK' => {
                        name => 'A1',
                        required => 1,
        ...
        # C1
        # If the Function of the Message (field 23G) is not CANC then field 13A Number
        # Identification, must be present and contain '515' in minimum one occurrence of the
        # subsequence A1 Linkages (Error code(s): E59).
        {
            func => sub {
                my $doc = shift;
                my $tree = _get_seq_tree(doc => $doc);
                ...
                unless (seq_key_value_exists($fields_a, '23G', 'CANC')) {
                    my $count = 0;
                    for my $b (@$branches_a1) {
                        if (seq_key_value_exists($b->{fields}, '13A', '515')) {
                            $count++;
                        }
                    }
                    return 0 if ($count < 1);
