package CyberFT::SWIFT::Types::MT547;
...
our $ValidationProfile = {
    ...
    rules => [
        ...
        # С5 Если в последовательности Е НЕ используется поле :22FВ::DBNM, то в сообщении обязательно
        # должны быть указаны агент-поставщик и место проведения расчетов: в одном из повторений
        # подпоследовательности Е1 «Стороны при расчетах» должно присутствовать поле :95a::DEAG, а
        # в другом повторении – поле :95a::PSET (Код ошибки Е91).
        {
            func => sub {
                my $doc = shift;
                my $tree = build_seq_tree($doc->data);
