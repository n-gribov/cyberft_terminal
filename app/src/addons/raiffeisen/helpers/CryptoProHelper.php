...
class CryptoProHelper
{
    ...
    private static function getContainers(): array
    {
        $output = shell_exec(static::CSPTEST_EXECUTABLE_PATH . ' -keyset -enum_cont -verifycontext -fqcn');
    ...
    {
        for ($i = 0; $i < 1000; $i++) {
            $path = $basePath . '/raiff.' . str_pad($i, 3, '0', STR_PAD_LEFT);
            if (!$fs->exists($path)) {
                return $path;
            }
        }
