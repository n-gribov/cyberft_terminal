...
class Sbbol2ClientTerminalSettingsXmlContent extends CyberXmlContent implements TypeCyberXmlContentInterface
{
    const DEFAULT_NS_PREFIX = 'sbbol';
    const ROOT_ELEMENT = 'Sbbol2ClientTerminalSettings';
    const DEFAULT_NS_URI = 'http://cyberft.ru/xsd/sbbol.01';

    /** @var Sbbol2ClientTerminalSettingsType */
    ...
    public function init()
    {
        ...
        if (is_null($this->_parent->_typeModel)) {
            $this->_typeModel = new Sbbol2ClientTerminalSettingsType();
        } else {
    ...
    public function fetchCustomer()
    {
        ...
        $customer = new Sbbol2ClientTerminalSettingsCustomer();
        foreach ($customer->attributes as $key => $value) {
    ...
    public function fetchAccounts()
    {
        ...
        foreach ($nodeList[0]->childNodes as $node) {
            $account = new Sbbol2ClientTerminalSettingsAccount();
            foreach ($account->attributes as $key => $value) {
                $domAttribute = $node->attributes->getNamedItem($key);
                if ($domAttribute) {
