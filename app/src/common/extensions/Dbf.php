...
class Dbf {
	//Private Properties
	private $raw, $version, $modifyDate, $recsNum, $fullHeaderLen, $recLen, $fieldsNum, $headerSize = 32, $src,
		$header, $recsCount;
	...
	public function __construct($src) {
		...
		//Check DBF Version
		if (!((ord($this->raw[0]) == 3) or (ord($this->raw[0]) == 131) or (ord($this->raw[0]) == 245)
			  or (ord($this->raw[0]) == 139))
		) {
			throw new Exception('Not a valid DBF file!');
		}
		$arrHeaderHex = [];
		...
		while ($i++ < $this->fieldsNum) {
			...
			$this->header[$i]['mem_addr']      = dechex(ord(fread($this->src, 1)))
	...
	function modifyDate() { return $this->modifyDate; }

	function recsNum() { return $this->recsNum; }

	function fullHeaderLen() { return $this->fullHeaderLen; }

	function recLen() { return $this->recLen; }

	function fieldsNum() { return $this->fieldsNum; }
