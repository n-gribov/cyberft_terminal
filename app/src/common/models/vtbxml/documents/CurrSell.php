 ...
 */
class CurrSell extends BSDocument
{
    const TYPE = 'CurrSell';
    const TYPE_ID = 7;
    const VERSIONS = [5, 6];
    const SIGNED_FIELDS_IDS_BY_VERSION = [
        5 => ['DOCUMENTDATE', 'DOCUMENTNUMBER', 'CUSTID', 'SENDEROFFICIALS', 'CUSTOMERNAME', 'KBOPID', 'CUSTOMERPROPERTYTYPE', 'CUSTOMERINN', 'CUSTOMEROKPO', 'CUSTOMERPLACE', 'CUSTOMERPLACETYPE', 'CUSTOMERADDRESS', 'CUSTOMERCOUNTRY', 'ACCOUNTRUR', 'ACCOUNTCURR', 'CUSTOMERTYPE', 'PHONEOFFICIAL', 'FAXOFFICIAL', 'REQUESTRATE', 'ACCOUNTDEBET', 'CURRCODEDEBET', 'AMOUNTDEBET', 'ACCOUNTCREDIT', 'CURRCODECREDIT', 'AMOUNTCREDIT', 'DEBETBANKBIC', 'DEBETBANKTYPE', 'DEBETBANKNAME', 'DEBETBANKPLACE', 'DEBETBANKPLACETYPE', 'DEBETBANKCORRACC', 'CREDITBANKBIC', 'CREDITBANKTYPE', 'CREDITBANKNAME', 'CREDITBANKPLACE', 'CREDITBANKCORRACC', 'CREDITBANKPLACETYPE', 'VALIDITYPERIOD', 'PAYMENTDETAILS', 'CHARGEACCOUNT', 'VALUEDATETYPE', 'DEALTYPE', 'GROUNDRECEIPTSBLOB', 'CURRENCYSOURCE', 'ACCOUNTDEBETTYPE', 'NUMBERCURRENCYBUY', 'DATECURRENCYBUY', 'TRANSFERACCOUNT', 'TRANSFERDOCUMENTNUMBER', 'TRANSFERDOCUMENTDATE', 'OPERCODE', 'OPERCODEDESCRIPTION', 'CURRDEALINQUIRYNUMBER', 'CURRDEALINQUIRYDATE', 'RESERVED', 'RESERVED1', 'REQUESTRATETYPE', 'CHARGETYPE', 'SUPPLYCONDITION', 'SUPPLYCONDITIONDATE', 'BANKAGREEMENT', 'CODEMESSAGE', 'MESSAGEFORBANK', 'BALANCEACCOUNT', 'BALANCEBANKBIC', 'BALANCEDEPINFO'],
        6 => ['DOCUMENTDATE', 'DOCUMENTNUMBER', 'CUSTID', 'SENDEROFFICIALS', 'CUSTOMERNAME', 'KBOPID', 'CUSTOMERPROPERTYTYPE', 'CUSTOMERINN', 'CUSTOMEROKPO', 'CUSTOMERPLACE', 'CUSTOMERPLACETYPE', 'CUSTOMERADDRESS', 'CUSTOMERCOUNTRY', 'ACCOUNTRUR', 'ACCOUNTCURR', 'CUSTOMERTYPE', 'PHONEOFFICIAL', 'FAXOFFICIAL', 'REQUESTRATE', 'ACCOUNTDEBET', 'CURRCODEDEBET', 'AMOUNTDEBET', 'ACCOUNTCREDIT', 'CURRCODECREDIT', 'AMOUNTCREDIT', 'DEBETBANKBIC', 'DEBETBANKTYPE', 'DEBETBANKNAME', 'DEBETBANKPLACE', 'DEBETBANKPLACETYPE', 'DEBETBANKCORRACC', 'CREDITBANKBIC', 'CREDITBANKTYPE', 'CREDITBANKNAME', 'CREDITBANKPLACE', 'CREDITBANKCORRACC', 'CREDITBANKPLACETYPE', 'VALIDITYPERIOD', 'PAYMENTDETAILS', 'CHARGEACCOUNT', 'VALUEDATETYPE', 'DEALTYPE', 'GROUNDRECEIPTSBLOB', 'CURRENCYSOURCE', 'ACCOUNTDEBETTYPE', 'NUMBERCURRENCYBUY', 'DATECURRENCYBUY', 'TRANSFERACCOUNT', 'TRANSFERDOCUMENTNUMBER', 'TRANSFERDOCUMENTDATE', 'OPERCODE', 'OPERCODEDESCRIPTION', 'RESERVED', 'RESERVED1', 'REQUESTRATETYPE', 'CHARGETYPE', 'SUPPLYCONDITION', 'SUPPLYCONDITIONDATE', 'BANKAGREEMENT', 'CODEMESSAGE', 'MESSAGEFORBANK', 'BALANCEACCOUNT', 'BALANCEBANKBIC', 'BALANCEDEPINFO'],
    ];
    ...
    /** @var CurrSellGroundReceipt[] Документы, обосновывающие сделку **/
    public $GROUNDRECEIPTSBLOB = [];

    /** @var string Источник происхождения валюты **/
    public $CURRENCYSOURCE;

    /** @var string Тип счета списания валюты: текущий, спец. транзитный, другое **/
    public $ACCOUNTDEBETTYPE;

    /** @var string Номер заявки на покупку (для обратной продажи) **/
    public $NUMBERCURRENCYBUY;

    /** @var \DateTime Дата первоначальной покупки валюты **/
    public $DATECURRENCYBUY;

    /** @var string Счет в банке, на который переведены средства для продажи **/
    public $TRANSFERACCOUNT;

    /** @var string Номер поручения на перевод на счет продажи **/
    public $TRANSFERDOCUMENTNUMBER;

    /** @var \DateTime Дата поручения на перевод на счет продажи **/
        ...
        return array_merge(
            ...
            [
                ...
                'GROUNDRECEIPTSBLOB' => new BlobTableField([
                    ...
                    'recordType'  => 'CurrSellGroundReceipt',
                    'versions'    => [5, 6],
                ]),
                'CURRENCYSOURCE' => new StringField([
                    'isRequired'  => false,
                    'isSigned'    => true,
                    'description' => 'Источник происхождения валюты',
                    'length'      => 255,
                    'versions'    => [5, 6],
                ]),
                'ACCOUNTDEBETTYPE' => new StringField([
                    'isRequired'  => false,
                    'isSigned'    => true,
                    'description' => 'Тип счета списания валюты: текущий, спец. транзитный, другое',
                    'length'      => 40,
                    'versions'    => [5, 6],
                ]),
                'NUMBERCURRENCYBUY' => new StringField([
                    'isRequired'  => false,
                    'isSigned'    => true,
                    'description' => 'Номер заявки на покупку (для обратной продажи)',
                    'length'      => 5,
                    'versions'    => [5, 6],
                ]),
                'DATECURRENCYBUY' => new DateField([
                    'isRequired'  => false,
                    'isSigned'    => true,
                    'description' => 'Дата первоначальной покупки валюты',
                    'versions'    => [5, 6],
                ]),
                'TRANSFERACCOUNT' => new StringField([
                    'isRequired'  => false,
                    'isSigned'    => true,
                    'description' => 'Счет в банке, на который переведены средства для продажи',
                    'length'      => 25,
                    'versions'    => [5, 6],
                ]),
                'TRANSFERDOCUMENTNUMBER' => new StringField([
                    'isRequired'  => false,
                    'isSigned'    => true,
                    'description' => 'Номер поручения на перевод на счет продажи',
                    'length'      => 15,
                    'versions'    => [5, 6],
                ]),
                'TRANSFERDOCUMENTDATE' => new DateField([
                    'isRequired'  => false,
                    'isSigned'    => true,
                    'description' => 'Дата поручения на перевод на счет продажи',
                    'versions'    => [5, 6],
                ]),
