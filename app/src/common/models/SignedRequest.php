...
class SignedRequest extends \common\base\Model
{
    ...
    public function validateSign($attribute, $params)
    {
        ...
        try {
            ...
            } elseif (CertManagerException::ERROR_UNKNOWN_CERT_ID === $e->getCode()) {
				$this->addError($attribute, Yii::t('app/cert', 'Requested certificate is not registered in the Terminal'));
            } elseif (CertManagerException::ERROR_BROKEN_CERT_PATH === $e->getCode()) {
                $this->addError($attribute, Yii::t('app/cert', 'Technical error. Key associated with the certificate was not found.'));
            }
        } catch (DbException $e) {
            $this->addError($attribute, Yii::t('app/cert', 'Technical error. The signature cannot be verified, please try again later. \n{msg}', [
				$e->getMessage()
			]));
        } catch (\Exception $e) {
            $this->addError($attribute, Yii::t('app/cert', 'Unknown signature verification error. \n{msg}', [
				$e->getMessage()
			]));
        }
